...
def ensure_server_ready():
    """Lazily wait for FastAPI on first tool call."""
    global _server_ready
    if not _server_ready:
        _server_ready = wait_for_server()
...
def create_mcp_server():
    ...
    async def list_calendars(min_access_role: str = None) -> str:
        ...
        try:
            ...
            response = _get("/calendars", params=params)
            if response.status_code != 200:
    ...
    async def find_events(calendar_id: str, time_min: str = None,
        ...
        try:
            ...
            if query:
                params["q"] = query
            response = _get(f"/calendars/{calendar_id}/events", params=params)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def create_event(calendar_id: str, summary: str, start_time: str,
        ...
        try:
            ...
            response = _post(f"/calendars/{calendar_id}/events", json=data)
    ...
    async def quick_add_event(calendar_id: str, text: str) -> str:
        ...
        """
        try:
            response = _post(f"/calendars/{calendar_id}/events/quickAdd", json={"text": text})
            if response.status_code != 201:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def update_event(calendar_id: str, event_id: str, summary: str = None,
        ...
        try:
            ...
            if location:
                data["location"] = location
            response = _patch(f"/calendars/{calendar_id}/events/{event_id}", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def delete_event(calendar_id: str, event_id: str) -> str:
        ...
        """
        try:
            response = _delete(f"/calendars/{calendar_id}/events/{event_id}")
    ...
    async def add_attendee(calendar_id: str, event_id: str, attendee_emails: List[str]) -> str:
        ...
        """
        try:
            response = _post(f"/calendars/{calendar_id}/events/{event_id}/attendees",
                             json={"attendee_emails": attendee_emails})
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def check_attendee_status(event_id: str, calendar_id: str = "primary",
        ...
        try:
            ...
            response = _post("/events/check_attendee_status", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def query_free_busy(calendar_ids: List[str], time_min: str, time_max: str) -> str:
        ...
        try:
            ...
            response = _post("/freeBusy", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def schedule_mutual(attendee_calendar_ids: List[str], time_min: str,
        ...
        try:
            ...
            response = _post("/schedule_mutual", json=data)
            if response.status_code != 201:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def analyze_busyness(time_min: str, time_max: str, calendar_id: str = "primary") -> str:
        ...
        """
        try:
            data = {"time_min": time_min, "time_max": time_max, "calendar_id": calendar_id}
            response = _post("/analyze_busyness", json=data)
            if response.status_code != 200:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
    ...
    async def create_calendar(summary: str) -> str:
        ...
        """
        try:
            response = _post("/calendars", json={"summary": summary})
            if response.status_code != 201:
                error_msg = f"Error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return json.dumps({"error": error_msg})
